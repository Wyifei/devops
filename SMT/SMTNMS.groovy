            ...
            steps {
                script {
                    ...
                    try {
                        ...
                        TOMCAT_WEBAPP_PATH=props.getProperty("tomcat.appnms.webapp.dir")
                        TOMCAT_APP_HOME=props.getProperty("tomcat.appnms.home.dir")
                        ...
                        for (def tomcatHost : tomcatHostArr) {
                            withCredentials([sshUserPrivateKey(credentialsId: "${CI_CREDENTIALS}",keyFileVariable: 'pem',usernameVariable: 'username')]) {
                                ...
                                sh """
                                    ...
                                    ssh -i ${pem} ${username}@${tomcatHost} 'echo "" > ${TOMCAT_USER_HOME}//tomcat-smt-nms-service/logs/catalina.out'
            ...
            steps {
                script {
                    ...
                    try {
                        ...
                        for (def tomcatHost : tomcatHostArr)  {
                                 withCredentials([sshUserPrivateKey(credentialsId: "${CI_CREDENTIALS}",keyFileVariable: 'pem',usernameVariable: 'username')]){
                                def curlCmd = """ssh -i ${pem} ${username}@${tomcatHost} 'curl -X GET -o /dev/null -w "%{http_code}" -s http://${tomcatHost}:9831/management-1.0/api/sys/resource/list'"""
                                println(curlCmd)
                                def responseCode =  sh script: curlCmd, returnStdout: true
                                if (responseCode != "200") {
                                    hasError = true
                                    echo "ERROR! Cannot access instance on ${tomcatHost}, URL: http://${tomcatHost}:9831/management-1.0/api/sys/resource/list"
                                }
